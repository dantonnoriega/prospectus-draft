...
\addcontentsline{toc}{section}{The Data Generating Process}
...
\addcontentsline{toc}{subsection}{Posing the Problem: The Ideal}
...
\begin{longtable}[]{@{}rcc@{}}
\caption{\label{tab:potential-outcomes} Expected potential
outcomes}\tabularnewline
\toprule
& Shopper \(r\) & Shopper \(u\)\tabularnewline
\midrule
\endfirsthead
\toprule
& Shopper \(r\) & Shopper \(u\)\tabularnewline
\midrule
\endhead
Store \(j\) with DUFB: \(y_{1ij}\) & \(\alpha + \beta_{r}\) &
\(\alpha + \beta_u\)\tabularnewline
Store \(j\) w/o DUFB: \(y_{0ij}\) & \(\alpha\) &
\(\alpha\)\tabularnewline
\bottomrule
\end{longtable}
