...
```{r score-plot, warning=FALSE, message=FALSE, echo=FALSE, eval=TRUE, fig.height=3, fig.cap = "Store Score vs Double Up Assignment"}
d <- score_controls(metric='geo')
dd <- d %>%
  dplyr::slice(1:23) %>%
  dplyr::mutate(assignment = 1 - control) %>%
  dplyr::select(score, assignment) %>%
  dplyr::mutate(`Double Up` = if_else(assignment == 1, 'Yes', 'No'))

ggplot2::qplot(x = score, y = assignment, data = dd, colour = `Double Up`) +
  theme_clean() +
