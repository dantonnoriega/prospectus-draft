...
y3 <- c(y2[1:7], sapply(y1[8:12], shock, c = 3, m = 2))
d <- y2[7] - y1[7]
y2 <- c(rep(NA,6), y1[7:12] + d)
df %<>%
...
ggplot(data = df, aes(x = month, y = dollars)) +
  ...
  theme_classic() +
  geom_vline(xintercept = 7, linetype = 2, color = 'gray') +
  geom_text(aes(7, 16, label = ' post', hjust = -.2), size = 3, color = 'gray') +
  geom_text(aes(7, 16, label = 'pre ', hjust = 1.2), size = 3, color = 'gray')
