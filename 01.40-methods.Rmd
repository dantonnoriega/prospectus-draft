...
**Continuous Dependent Variables and Corner Solutions**
...
In the next subsection, I will formally introduce the different modeling structures I intend to use in my paper. I will start with the Tobit model and end with the more complex MDCEV model. The share goal, across all models, is the best possible measurement of the impact the DUFB incentive has on fruit and vegetable expenditures between participating and non-participating stores.
...
Let $y_{ijt}$ be total FV expenditures for transaction $i$ in store $j$ from day $t$. The DD regression is

$$
y_{ijt} = \gamma_j + \lambda_t + \delta D_{jt} + \bm{x'_{ijt} \beta} + \epsilon_{ijt}
$$

where $\gamma_j$ are store fixed-effects, $\lambda_t$ capture daily (time) effects, $D_{jt}$ indicates store $j$ participation in DUFB on day $t$, and $\bm{x'_{ijt} \beta}$ is a vector of observable characteristics for each transaction $i$ in store $j$ on day $t$. Idiosyncratic errors are represented by $\epsilon_{ijt}$.

Note that $\gamma_j$ captures the assignment group, as well as all other time-invariant store-level characteristics. The daily effects are included given the amount of data captured at the daily level. I anticipate day-of-the-week and week-of-the-month to matter when analyzing purchasing behavior. $\bm{x'_{ijt}
