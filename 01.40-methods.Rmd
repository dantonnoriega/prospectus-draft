...
The limitation of Tobit and other hurdle models is they assume optimal demand of a single good. Ideally, expenditure on different types of goods better captures the decisions and purchasing behavior of customers within a grocery store. That is, instead of collapsing all expenditure on FVs into one outcome variable, the model would allow for customers to optimize across *multiple* goods of different types, while also allowing corner solutions.

@dubin_econometric_1984 construct a *discrete-continuous* model were consumers can select from multiple goods/options but that they are mutually exclusive, perfect substitutes. Selecting one good/option means that one cannot be observed buying/consuming the other. The Kuhn-Tucker based model developed by @wales_estimation_1983, on the other hand, allows non-zero consumption across multiple goods. @kim_modeling_2002 solved the intractability of the @wales_estimation_1983 model and @bhat_multiple_2005 simplified both to be more realistically applicable. 

To model expenditures along more than one dimension (fruits and vegetables), I will also implement the *multiple* discrete-continuous model extreme value (MDCEV) model introduced by @bhat_multiple_2005. For any observed trip to the store, zero-expenditure in FV implies non-zero expenditure for some non-FV. Ignoring this seems unwise and I plan, at a minimum, to implement a model optimize along two dimensions---FV and non-FV. 

In the next subsection, I will formally introduce each model I intend to estimate, with increasing levels of complexity.


### Overview of Proposed Methods {-}


The Difference-in-Differences (DD) regression framework will be present in each of the models of my main analysis. The DD analysis makes use of as much data a possible---17 *treated* stores and the 15 *control* stores. I will assume the selection problem outline in the [Store Selection Section](#store-selection-1) can be controlled using store-level fixed-effects. That is, I will assume the DUFB treatment is strictly exogenous conditional on any time-invariant (observed and unobserved) store-level characteristics.
...
### Main Analyses: Difference-in-Differences (DD) {-}
